...
const Leveraging = ({ isMobile }) => {
    const leveraging = useGLTF("https://res.cloudinary.com/dl9ey6o4d/image/upload/v1773320814/Soniclevitation_ylmu0f.glb");
    const meshRef = useRef();
    const [isHovered, setIsHovered] = useState(false);

    // Apply gradient material to all meshes
    useEffect(() => {
        leveraging.scene.traverse((child) => {
            if (child.isMesh) {
    ...
    return (
        ...
        >
            ...
            <primitive
                object={leveraging.scene}
                scale={isMobile ? 0.009 : 0.009}
                position={[0, 0, -1]}
...
export default LeveragingLight;